resource "aws_eks_cluster" "team4_clu" {
  name     = "team4-clu"
  role_arn = aws_iam_role.cluster_role.arn
  version  = "1.27"
  vpc_config {
    subnet_ids              = concat(aws_subnet.team4_pub_[*].id, aws_subnet.team4_web_[*].id, aws_subnet.team4_was_[*].id, aws_subnet.team4_db_[*].id)
...
resource "aws_eks_addon" "eks_cni" {
  cluster_name                = aws_eks_cluster.team4_clu.name
  addon_name                  = "vpc-cni"
